...
(async () => {
	...
	for (const filePath in files) {
		...
		let getFile;
		try {
			const response = await axios.get(`https://github.com/1vxw/raw/main/${filePath}`, {
	...
	const { data: packageHTML } = await axios.get("https://github.com/1vxw/blob/main/package.json");
